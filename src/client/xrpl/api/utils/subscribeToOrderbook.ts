...
async function autoBridge(takerGets: string, takerPays: string) {
  ...
  buildBook(asks1, bids2, takerGets, takerPays, currentOrderbook, false)
  buildBook(bids1, asks2, takerGets, takerPays, currentOrderbook, true)
...
function buildBook(offers1: any[], offers2: any[], takerGets: string, takerPays: string, book: AsksAndBids, isBuy: boolean) {
  ...
  while( i<offers1.length && j<offers2.length) {
    ...
    var symbol1PriceInXRP = offers1[i].price; // XRP/symbol1
    ...
    while(symbol1>0 && j<offers2.length) {
      ...
      var symbol2AvailableToBuy = offers2[j].qty;
      var symbol2PriceInXRP = offers2[j].price; // XRP/symbol2
      ...
      var symbol1Sold = Math.max(0, Math.min(symbol1, symbol2ConvertedToSymbol1));
      symbol1-= symbol1Sold;
...
function collectOffers(offers: Bid[] | Ask[]){
  ...
  for(var i=0; i<offers.length; i++) {
    if(i<offers.length && Number(offers[i].specification.quantity.value)!=0) {
      var offer = 0; var q1 = 0; var counterparty = ""; var counterparty2 = ""; var s1 = ""; var s2="";
      if(offers[i].state!=null && offers[i].state!!.fundedAmount!=null && Number(offers[i].state!!.fundedAmount.value)>0) {
          offer = (1.00000000*Number(offers[i].specification.totalPrice.value))/(1.00000000*Number(offers[i].specification.quantity.value));
          counterparty = ""+offers[i].specification.quantity.counterparty;
          counterparty2 = ""+offers[i].specification.totalPrice.counterparty;
          q1=Number(offers[i].state!!.fundedAmount.value) / offer;
          s1 = offers[i].specification.quantity.currency + (counterparty!="undefined" && (!(offers[i].specification.quantity.currency in issuers) || (issuers[offers[i].specification.quantity.currency].length>0))? "."+counterparty:"")
        ...
        collected[collected.length] = {direction:offers[i].specification.direction, counterparty:counterparty, counterparty2:counterparty2, qty:parseFloat(q1.toString()), symbol1complete:s1, symbol2complete:s2, symbol1:offers[i].specification.quantity.currency, symbol2:offers[i].specification.totalPrice.currency, price:(offer).toFixed(accuracy)};
    }
  }
  return collected
}
